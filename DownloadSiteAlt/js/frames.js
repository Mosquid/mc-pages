...
$(document).ready(function() {
    ...
    if( $('#banner-switcher').length ) {
        $('#banner-switcher .switch-links li a.place-link').on('click', function(){
            ...
            if( leftPos < 0 ) 
                return false;
            if( alreadyOpen && !$(this).hasClass('show-child') ) {
                var opened = $(this).parents('.switch-links').find('.show-child');
                opened.removeClass('show-child');
                opened.parent().find('ul').removeAttr('style');
            }
            ...
            if( !$(this).hasClass('haschild') ) {
                ...
                if( $(this).hasClass('show-child') ) {
                    ...
                    $(this).parents('li').find('.child').removeAttr('style');
                } else {
                    $(this).addClass('show-child');
                    child.css({'marginRight': -childWidth-thWidth});
                }
